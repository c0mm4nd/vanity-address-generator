...
use sha2::Digest; // Add the Digest trait import here
...
pub fn run_gpu_tron_miner(platform_idx: i32, regex_str: &str) -> Result<(String, String), String> {
    // Load kernel source
    let kernel_path = Path::new("src/cl/tron_kernel.cl");
    let kernel_source = match read_to_string(&kernel_path) {
        Ok(source) => source,
        Err(e) => return Err(format!("Failed to read Tron kernel source: {}", e))
    };
    ...
    let addresses_buffer = match Buffer::<u8>::builder()
        .queue(queue.clone())
        .flags(MemFlags::WRITE_ONLY)
        .len(BATCH_SIZE * 20) // 20 bytes per address
        .build() {
            Ok(b) => b,
            Err(e) => return Err(format!("Failed to create addresses buffer: {}", e))
        };
    ...
    println!("OpenCL initialization complete. Starting Tron GPU mining...");
    println!("Looking for Tron addresses matching regex: {}", regex_str);
    ...
    loop {
        ...
        if found_index[0] != u32::MAX {
            ...
            let found_idx = found_index[0] as usize;
            let address_bytes = &addresses[found_idx * 20..(found_idx + 1) * 20];
            let private_key_bytes = &private_keys[found_idx * 32..(found_idx + 1) * 32];

            // For Tron addresses, we need to format as base58
            // In a real implementation, this would convert the address correctly
            // For now, we'll use a placeholder and convert it properly on the CPU
            // Generate proper Tron address from the raw bytes
            let mut tron_bytes = vec![0x41]; // Tron prefix
            tron_bytes.extend_from_slice(address_bytes);
            
            // Calculate checksum (double SHA-256 hash of the address bytes)
            ...
            found_address = bs58::encode(tron_bytes).into_string();
            found_private_key = hex::encode(private_key_bytes);

            println!("Found matching Tron address after {} batches!", batch_count);
            println!("Address: {}", found_address);
